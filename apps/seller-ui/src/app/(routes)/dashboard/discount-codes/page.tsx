...
const Page = () => {
  ...
  const deleteDiscountCodeMuatation = useMutation({
  ...
  const onSubmit = (data: any) => {
    if (discountCodes?.length >= 8) {
      toast.error("You can add maximum 8 discount codes");
      return;
    }
    ...
    <div className="w-full min-h-screen p-8">
              ...
              <div className="mt-4">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <select
                      {...field}
                      className="w-full !border cursor-pointer !outline-none border-gray-700 p-2 rounded-md text-white bg-transparent"
                    >
      ...
      {showDeleteModal && selectedDiscount && (
        <DeleteDiscountCodeModal
          ...
          onConfirm={() =>
            deleteDiscountCodeMuatation.mutate(selectedDiscount?.id)
          }
        />
      )}
    </div>
